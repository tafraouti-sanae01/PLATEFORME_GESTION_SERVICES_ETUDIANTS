...
function getApiBase(): string {
  ...
  // Le proxy redirigera /api/* vers http://localhost/PLATEFORME_GESTION_SERVICES_ETUDIANTS/backend
...
export async function updateRequestStatus(id: string, status: "accepted" | "rejected" | "pending", adminId?: string) {
...
export async function getStudentDemands(email: string, apogee: string, cin: string): Promise<StudentDemand[]> {
  return fetchJson<StudentDemand[]>("/api/students/demands", {
    method: "POST",
    body: JSON.stringify({ email, apogee, cin }),
  });
}
