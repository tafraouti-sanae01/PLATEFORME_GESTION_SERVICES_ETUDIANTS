...
interface AppContextType {
  ...
  updateRequestStatus: (id: string, status: "accepted" | "rejected" | "pending", adminId?: string) => Promise<void>;
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const updateRequestStatusMutation = useMutation({
    mutationFn: ({ id, status, adminId }: { id: string; status: "accepted" | "rejected" | "pending"; adminId?: string }) =>
      updateRequestStatusAPI(id, status, adminId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["requests"] });
    },
  });
  ...
  const updateRequestStatus = async (id: string, status: "accepted" | "rejected" | "pending", adminId?: string) => {
    try {
      await updateRequestStatusMutation.mutateAsync({ id, status, adminId: adminId || currentAdmin?.id });
    } catch (error) {
